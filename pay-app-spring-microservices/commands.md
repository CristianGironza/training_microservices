...
docker run -p 3306:3306 --name mysql --network distribuidos -e MYSQL_ROOT_PASSWORD=mysql -e MYSQL_DATABASE=db_operation -d cgironza/mysql:0.1.0
...
docker run -d -p 8888:8888 --network distribuidos --name app-config cgironza/appconfig:0.1.0
